...
def recv_line(sock: socket.socket, max_len: int = MAX_FILENAME_LEN) -> bytes:
    ...
    while True:
        ...
        data += b
        if (len(data) > max_len):
            raise ValueError("Received line too long")
...
def handle_client(conn: socket.socket, outdir: str) -> None:
    ...
    try:
        ...
        # TODO: write your code here.
        while len(hdr) < 8:
            part = conn.recv(8 - len(hdr))
            if part == b'':
                raise ConnectionError("Socket closed while receiving header")
            hdr += part
        ...
        try:
            with open(dest_path, 'wb') as f:
                while remaining > 0:
                    ...
                    if chunk == b'':
                        raise ConnectionError("Socket closed while receiving file payload")
                    f.write(chunk)
                    remaining -= len(chunk)
                f.flush()
        except Exception:
...
def run_client(server_ip: str, port: int, file_path: str, ipv6: bool) -> int:
    ...
    try:
        ...
        try:
            ...
            # 1) Send filename line.
            sock.sendall(filename.encode('utf-8') + b'\n')

            # 2) Wait for server OK/ERR.
            resp = recv_line(sock, max_len=16)
            if resp == b'ERR':
                return 1
            if resp != b'OK':
                return 255

            # 3) Send 8-byte length.
            sock.sendall(struct.pack('!Q', file_size))

            # 4) Stream file content in chunks.
            ...
            # 5) Wait final ACK.
            final = recv_line(sock, max_len=16)
            if final == b'OK':
                return 0
            return 255
